...
fn try_follow_breach_u128(
    ...
    if value == 1 {
        ...
        row.notes = "bounded u128 witness reached terminal 1".to_string();
    } else {
    ...
    for next_depth in (start_depth + 1)..=config.depth {
        ...
        for key in order.iter() {
            ...
            let witness = witnesses
                .get(key)
                .expect("witness missing for merged census key");
            if let Some(max_deficit) = max_deficit_for_c(config.c) {
                ...
                for d_next in 0..=max_deficit {
                    ...
                    if !next_counts.contains_key(&next_key) {
                        ...
                        next_witnesses.insert(next_key, update_witness(witness, a));
                    }
                    next_counts
                        .get_mut(&next_key)
...
fn run_census_lean(config: &Config, started: Instant) -> CensusResult {
    ...
    for next_depth in (start_depth + 1)..=config.depth {
        ...
        {
            eprintln!(
                "lock3-census-progress engine=rust C={} depth={}/{} branches={} merged={} valid1={} exact_depth_valid1={} compatible_signatures={} live_valid1={} valid1_births={} valid1_deaths={} max_valid1_lifetime={} valid1_persisted={} live_valid1_lineages={} lineage_births={} lineage_deaths={} max_lineage_lifetime={} ever_seen_valid1={} valid_residues={} mod={} lifts={} longest_plateau={} max_bits={} rss_kb={} peak_rss_kb={} elapsed_sec={:.1} notes={}",
                config.c,
                next_depth,
                config.depth,
                row.symbolic_branch_count.to_decimal(),
