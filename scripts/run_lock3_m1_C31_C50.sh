...
LOG_DIR="$ROOT/logs"
SUMMARY="$LOG_DIR/lock3_m1_C31_C50_${STAMP}.summary.log"

mkdir -p "$LOG_DIR"
exec >> "$SUMMARY" 2>&1

echo "lock3 m1 C31-C50 batch start stamp=$STAMP"
echo "root=$ROOT"
echo "bin=$BIN"

for C in $(seq 31 50); do
...
echo "lock3 m1 C31-C50 batch complete stamp=$STAMP"
